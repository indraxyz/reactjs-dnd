...
const Column = ({
  ...
  const handleDragOver = (e: DragEventReact) => {
    e.preventDefault();
  ...
  const highlightIndicator = (e: DragEventReact) => {
    const indicators = getIndicators();
...
const Trash = ({
  ...
  return (
    <div
      ...
      onDragOver={handleDragOver}
      onDragLeave={handleDragLeave}
      className={`mt-10 grid h-28 w-28 shrink-0 place-content-center rounded border text-3xl ${
        active
          ? "border-red-800 bg-red-800/20 text-red-500"
          : "border-neutral-500 bg-neutral-500/20 text-neutral-500"
      }`}
    >
      {active ? <FiX className="animate-bounce" /> : <FiTrash />}
    </div>
  );
};
